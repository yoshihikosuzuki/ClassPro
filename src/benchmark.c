...
{ struct rusage    now;
  ...
  if (usecs >= 60)
    fprintf (f,"  %d:%02d.%03d user",usecs/60,usecs%60,umics/1000);
  else
    fprintf (f,"  %d.%03d user",usecs,umics/1000);
  ...
  if (ssecs >= 60)
    fprintf (f,"  %d:%02d.%03d sys",ssecs/60,ssecs%60,smics/1000);
  else
    fprintf (f,"  %d.%03d sys",ssecs,smics/1000);
  ...
  if (tsecs >= 60)
    fprintf (f,"  %d:%02d.%03d wall",tsecs/60,tsecs%60,tmics/1000);
  else
    fprintf (f,"  %d.%03d wall",tsecs,tmics/1000);
